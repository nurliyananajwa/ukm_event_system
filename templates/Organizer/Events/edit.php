 ...
 * @var array $guestRows      array of guests rows for this event
 * @var array $docRows        array of documents rows for this event
 * @var \App\Model\Entity\Request|null $request
 */

$this->assign('title', 'Edit Event');
...
$defaultGuests = !empty($guestRows)
    ? $guestRows
    : [['guest_type_id' => '', 'guest_name' => '', 'designation' => '', 'organization' => '']];

$defaultDocs = !empty($docRows)
    ? $docRows
    : [['doc_type_id' => '', 'company_info' => '', 'file_path' => '']];
?>

<style>

.form-head{ margin-bottom:14px; display:flex; justify-content:space-between; gap:12px; align-items:flex-start; }
...
.section-title{
    font-size:14px; font-weight:950; letter-spacing:.6px; color:#0f172a;
    margin-bottom:14px; border-left:4px solid #2563eb; padding-left:10px;
    text-transform: uppercase;
}
.field{ margin-bottom:14px; }
.field label{
    display:block; font-size:12px; font-weight:900; color:#1e293b;
...
.repeat-row{
    ...
    padding:12px; border-radius:14px; border:1px solid #e5e7eb; background:#f8fafc;
}
.repeat-row .mini{ display:flex; flex-direction:column; gap:6px; }
.repeat-row .mini label{ font-size:11px; letter-spacing:.3px; margin-bottom:5px; color:#475569; font-weight:900; }
.repeat-row .mini input, .repeat-row .mini select{
    padding:10px 12px; border-radius:14px; border:1px solid #cbd5e1; background:#fff; height:44px;
}
.repeat-row .actions-col{
    display:flex; justify-content:flex-end; align-items:flex-start; padding-top:22px;
}

.repeat-row.doc-row{
    grid-template-columns: 220px 1fr 1.2fr 110px;
    padding:14px; border-radius:16px; border:1px solid #e5e7eb; background:#f8fafc;
}
...
@media (max-width: 980px){
    ...
    .repeat-row.doc-row{ grid-template-columns: 1fr; }
}
...
@media print{
    ...
    .content{ border:none !important; box-shadow:none !important; padding:0 !important; }
...
<div class="form-head">
    <div>
        <h2>Edit Event</h2>
        <p>Update the details below. You can <b>Print / Save as PDF</b> after saving.</p>
    </div>

    <div class="no-print" style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
        <?= $this->Html->link('Back', ['prefix'=>'Organizer','controller'=>'Events','action'=>'view', $event->id], ['class'=>'btn2']) ?>
        <button type="button" class="btn2" onclick="window.print()">Print / Save as PDF</button>
    </div>
</div>
...
<div class="card">
    ...
    <div class="row">
        <div class="field">
            <?= $this->Form->control('start_date', ['label'=>'Start Date','type'=>'date','required'=>true]) ?>
        </div>
        <div class="field">
            <?= $this->Form->control('end_date', ['label'=>'End Date (Optional)','type'=>'date']) ?>
        </div>
    </div>

    <div class="row">
        <div class="field">
            <?= $this->Form->control('time_start', ['label'=>'Start Time','type'=>'time','required'=>true]) ?>
        </div>
        <div class="field">
            <?= $this->Form->control('time_end', ['label'=>'End Time','type'=>'time','required'=>true]) ?>
        </div>
    </div>
        ...
        <?php foreach ($defaultGuests as $i => $g): ?>
            <div class="repeat-row guest-row">
                ...
                <div class="mini">
                    ...
                    <input type="text"
                           name="guests[<?= (int)$i ?>][organization]"
                           value="<?= h((string)($g['organization'] ?? '')) ?>"
                           placeholder="e.g., UKM / Company name">
                </div>
    ...
    <div class="repeat-wrap" id="docWrap">
        <?php foreach ($defaultDocs as $i => $d): ?>
            <div class="repeat-row doc-row">
                <input type="hidden"
                       name="documents[<?= (int)$i ?>][id]"
                       value="<?= h((string)($d['id'] ?? '')) ?>">

                <input type="hidden"
                       name="documents[<?= (int)$i ?>][existing_file_path]"
                       value="<?= h((string)($d['file_path'] ?? '')) ?>">
                ...
                <div class="mini">
                    ...
                    <input type="text"
                           name="documents[<?= (int)$i ?>][company_info]"
                ...
                <div class="mini">
                    ...
                    <?php if (!empty($d['file_path'])): ?>
                        <div class="help">
                            Current file: <b><?= h(basename((string)$d['file_path'])) ?></b><br>
                            <span style="color:#64748b;">(Leave upload empty to keep current file)</span>
                        </div>
                    <?php endif; ?>
                </div>
...
<div class="card">
    ...
    <div class="field">
        ...
        <input type="text"
               ...
               required
               value="<?= h((string)$requestData['requested_by']) ?>"
               placeholder="Full name">
    </div>
    ...
    <div class="row">
        <div class="field">
            ...
            <input type="text"
                   ...
                   value="<?= h((string)$requestData['position']) ?>"
        ...
        <div class="field">
            ...
            <input type="text"
                   ...
                   value="<?= h((string)$requestData['phone_number']) ?>"
    ...
    <div class="actions no-print">
        <?= $this->Html->link(
            'Cancel',
            ['prefix'=>'Organizer','controller'=>'Events','action'=>'view', $event->id],
            ['class'=>'btn2']
        ) ?>
...
function addGuestRow(){
    ...
    div.innerHTML = `
        ...
        <div class="mini"><label>Guest Name</label><input type="text" name="guests[${guestIndex}][guest_name]" placeholder="Full name"></div>
        <div class="mini"><label>Designation</label><input type="text" name="guests[${guestIndex}][designation]" placeholder="e.g., Director / Dean"></div>
        <div class="mini"><label>Organization</label><input type="text" name="guests[${guestIndex}][organization]" placeholder="e.g., UKM / Company name"></div>
        <div class="actions-col no-print"><button type="button" class="btn2 danger" onclick="removeRow(this)">Remove</button></div>
    `;
...
function addDocRow(){
    ...
    div.innerHTML = `
        ...
        <div class="mini"><label>Company / Partner Info</label><input type="text" name="documents[${docIndex}][company_info]" placeholder="Company / partner details (optional)"></div>
        <div class="mini"><label>Upload File (Optional)</label><input type="file" name="documents[${docIndex}][file]"></div>
        <div class="actions-col no-print"><button type="button" class="btn2 danger" onclick="removeRow(this)">Remove</button></div>
    `;
    wrap.appendChild(div);
    docIndex++;
